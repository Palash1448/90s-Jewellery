...
export const ProductHero: React.FC<ProductHeroProps> = ({ product }) => {
  ...
  return (
    <div className="flex flex-col space-y-4 sm:space-y-5">
      ...
      {/* Pricing Block */}
      <div className="p-4 rounded-xl bg-gradient-to-r from-[#FAF6EE] to-[#F5ECE0] border border-[#E8DCBE]">
        <div className="flex items-baseline gap-3">
          <span className="font-display font-bold text-3xl sm:text-4xl text-[#1E1A17]">
            ₹{product.price.toLocaleString('en-IN')}
          </span>

          {product.mrp > product.price && (
            <>
              <span className="text-base sm:text-lg text-[#8C8074] line-through font-medium">
                ₹{product.mrp.toLocaleString('en-IN')}
              </span>
              <span className="bg-[#8C2D3B] text-white text-xs font-bold px-2 py-0.5 rounded shadow-sm">
        ...
        <p className="text-[11px] text-[#6E6458] mt-1.5 flex items-center gap-1">
      ...
      {/* Short Description */}
      <p className="text-sm text-[#473E35] leading-relaxed">
      ...
      <div className="py-1">
        {isOutOfStock ? (
          <div className="flex items-center gap-2 text-rose-700 font-semibold text-sm bg-rose-50 px-3.5 py-2 rounded-lg border border-rose-200">
            <AlertTriangle className="w-4 h-4" />
            <span>Currently Out of Stock. Inquire on WhatsApp for next batch availability.</span>
          </div>
        ) : isLowStock ? (
      ...
      {isAvailable && (
        <div className="flex items-center gap-4 pt-1">
          ...
          <div className="flex items-center border border-[#D0C5B4] rounded-lg bg-white overflow-hidden shadow-sm">
            <button
              onClick={decrementQty}
              disabled={quantity <= 1}
              className="p-2 hover:bg-[#F3EDE4] text-[#1E1A17] disabled:opacity-30 transition-colors"
            >
              <Minus className="w-3.5 h-3.5" />
            </button>
            <span className="px-4 py-1 text-sm font-bold text-[#1E1A17] min-w-[36px] text-center">
              {quantity}
            </span>
            <button
      ...
      <div className="flex flex-col gap-3 pt-2">
        {isAvailable ? (
          <button
            ...
            className="w-full py-4 px-6 rounded-xl font-display font-bold text-base sm:text-lg text-white bg-[#1E1A17] hover:bg-[#0F0D0B] active:scale-[0.99] transition-all shadow-lg flex items-center justify-center gap-2 group cursor-pointer"
          >
            <ShoppingBag className="w-5 h-5 group-hover:scale-110 transition-transform text-[#D4AF37]" />
            <span>BUY NOW — ₹{(product.price * quantity).toLocaleString('en-IN')}</span>
          </button>
        ) : (
          <button
            disabled
            className="w-full py-3.5 px-6 rounded-xl font-display font-bold text-base text-gray-500 bg-gray-200 cursor-not-allowed"
          >
            OUT OF STOCK
          </button>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          ...
          <button
            ...
            type="button"
            className="w-full py-3 px-4 rounded-xl border border-[#D9CFBE] bg-[#FAF8F5] hover:bg-[#F3ECE0] text-[#1E1A17] font-semibold text-xs sm:text-sm flex items-center justify-center gap-2 transition-all active:scale-[0.98] shadow-2xs cursor-pointer"
          >
